...
class Robot(object):
    def __init__(self, addr=0x60, left_id=1, right_id=2, left_2_id=3, right_2_id=4, left_trim=0, right_trim=0,  left_2_trim=0, right_2_trim=0,
                 stop_at_exit=True):
        ...
        self._left_2 = self._mh.getMotor(left_2_id)
        self._right_2 = self._mh.getMotor(right_2_id)
        self._left_2_trim = left_2_trim
        self._right_2_trim = right_2_trim
        # Start with motors turned off.
        self._left_2.run(Adafruit_MotorHAT.RELEASE)
        self._right_2.run(Adafruit_MotorHAT.RELEASE)        
    ...
    def _left_2_speed(self, speed):
        """Set the speed of the left motor, taking into account its trim offset.
        """
        assert 0 <= speed <= 255, 'Speed must be a value between 0 to 255 inclusive!'
        speed += self._left_2_trim
        speed = max(0, min(255, speed))  # Constrain speed to 0-255 after trimming.
        self._left_2.setSpeed(speed)

    def _right_2_speed(self, speed):
        """Set the speed of the right motor, taking into account its trim offset.
        """
        assert 0 <= speed <= 255, 'Speed must be a value between 0 to 255 inclusive!'
        speed += self._right_2_trim
        speed = max(0, min(255, speed))  # Constrain speed to 0-255 after trimming.
        self._right_2.setSpeed(speed)    
    ...
    def forward(self, speed, seconds=None):
        ...
        self._left_2_speed(speed)
        self._right_2_speed(speed)
        self._left_2.run(Adafruit_MotorHAT.FORWARD)
        self._right_2.run(Adafruit_MotorHAT.FORWARD)        
    ...
    def backward(self, speed, seconds=None):
        ...
        self._left_2_speed(speed)
        self._right_2_speed(speed)
        self._left_2.run(Adafruit_MotorHAT.BACKWARD)
        self._right_2.run(Adafruit_MotorHAT.BACKWARD) 
